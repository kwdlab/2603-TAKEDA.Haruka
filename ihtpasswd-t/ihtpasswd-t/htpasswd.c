...
#include <errno.h>
#include <locale.h>
/* H.Takeda: NFC 正規化のため utf8proc を使用 */
#include <utf8proc.h>
...
static char *convert_to_utf8(const char *input, size_t len);
...
static void usage(void)
{
    apr_file_printf(errfile, "Usage:" NL
        "\ihtpasswd [-cimB25dpsDv] [-C cost] [-r rounds] passwordfile username" NL
        "\ihtpasswd -b[cmB25dpsDv] [-C cost] [-r rounds] passwordfile username password" NL
        NL
        "\ihtpasswd -n[imB25dps] [-C cost] [-r rounds] username" NL
        "\ihtpasswd -nb[mB25dps] [-C cost] [-r rounds] username password" NL
                       ...
                       char **pwfilename)
{
    ...
    *user = apr_pstrdup(pool, argv[i++]);
    ...
    /*H.Takeda*/
    char *utf8_user = convert_to_utf8(*user, strlen(*user));
    if (utf8_user != NULL) {
      // *userが指していたメモリ領域を解放
        *user = apr_pstrdup(pool, utf8_user);
        free(utf8_user);
    } else {
      // conver_to_utf8が失敗したとき
	fprintf(stderr, "Error: UTF-8 conversion failed for username.\n");
   	 exit(EXIT_FAILURE);
    }
//安全性確保のため;を拒否する
...
//パスワードがコマンドライン引数として与えられた場合の処理
    if (ctx->passwd_src == PW_ARG) {
        ...
        /*H.Takeda*/
//パスワードもUTF-8に
        char *utf8_pw = convert_to_utf8(ctx->passwd, strlen(ctx->passwd));
        if (utf8_pw) {
            ctx->passwd = apr_pstrdup(pool, utf8_pw);
            free(utf8_pw);
        } else {
    fprintf(stderr, "Error: UTF-8 conversion failed for password.\n");
    exit(EXIT_FAILURE);
    }
 }
}
...
//RFC 7617に従ってUTF-8に統一するための前処理関数
static char *convert_to_utf8(const char *input, size_t len)
{
//入力チェック
    if (!input || len == 0) {
        fprintf(stderr, "convert_to_utf8: input is NULL or length is zero\n");
        return NULL;
    }
//文字コード自動判別の準備
    uchardet_t ud = uchardet_new();
    if (!ud) {
        fprintf(stderr, "uchardet_new() failed\n");
        return NULL;
    }
//文字コード推定処理
    if (uchardet_handle_data(ud, input, len) != 0) {
        fprintf(stderr, "uchardet_handle_data() failed\n");
        uchardet_delete(ud);
        return NULL;
    }
...
//判別結果の取得とフォールバック
    ...
    if (!charset || strlen(charset) == 0) {
        fprintf(stderr, "Charset detection failed. Using fallback: UTF-8\n");
        charset = "UTF-8";
    }
//文字コード名の正規化
    char normalized_charset[64];
    strncpy(normalized_charset, charset, sizeof(normalized_charset) - 1);
    normalized_charset[sizeof(normalized_charset) - 1] = '\0';
    for (char *p = normalized_charset; *p; ++p)
...
//入力バッファの複製
    ...
    size_t outbytesleft = len * 4 + 1;
    char *inbuf_copy = malloc(len + 1);
    if (!inbuf_copy) {
        perror("malloc");
        iconv_close(cd);
        uchardet_delete(ud);
...
//入出力バッファ準備
    memcpy(inbuf_copy, input, len);
    inbuf_copy[len] = '\0';
    char *inbuf = inbuf_copy;
...
//実際のUTF-8変換
    ...
    *outbuf = '\0';
    ...
    iconv_close(cd);
    uchardet_delete(ud);
  /* H.Takeda: ここから正規化 (NFC) を行う
     * RFC 準拠のため、UTF-8 に変換したあとに NFC 正規化を必ず適用する。
     * utf8proc のヘルパ関数を使うと簡潔に実装できる*/
       utf8proc_uint8_t *normalized = utf8proc_NFC((const utf8proc_uint8_t *)output);
        if (!normalized) {
            fprintf(stderr, "utf8proc_NFC() failed, returning original UTF-8 string\n");
	    free(output);
	    exit(1); // 強制終了
        }

        free(output);// 正常時はoutputを捨てる
        /* normalized は malloc されている → 呼び出し側がfree */
